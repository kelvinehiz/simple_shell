...
/**
 * _getln -  Reads an entire line from stream,
 * storing the address of the buffer containing the text into *b.
 *
 ...
 */
size_t _getln(char **line, size_t *n, __attribute__((unused))FILE * stream)
{
	...
	*line = buffer;
	...
	{ fflush(stdout); /* Flush the stdout in every iteratin*/
		...
		if (c == EOF || c == '\n') /* If we hit EOF, replace it with /1 and return */
		{ *(buffer + index) = '\0';
			if (c == EOF)
				return (-1);
			else
		...
		{ bufsize += 1024;
			buffer = _realloct(buffer, bufsize - 1024, bufsize);
			if (alloc_error(buffer) == -1)
				return (1);
		}
	}
